...
int main(void) {
   ...
   for (;;) {
      ...
      /* receive the message */
    //  bytes_recd = recv(sock_connection, (char*)header, STRING_SIZE, 0);
      bytes_recd2 = recv(sock_connection, filename, STRING_SIZE, 0);
      header = ntohs(header);
    ...
    do {
        count++;
        fgets(line, STRING_SIZE, (FILE*)file);
        msg_len = strlen(line);
        /* send message */
        header++;
        header = htons(header);
    //    bytes_sent = send(sock_connection, (char*)header, msg_len, 0);
        bytes_sent2 = send(sock_connection, line, msg_len, 0);
    } while ((msg_len != 0) && (count < 50));
}
      /* close the socket */
      fclose(file);
